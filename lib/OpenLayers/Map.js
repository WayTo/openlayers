    ...
    moveTo: function(lonlat, zoom, options) {
        ...
        if (zoomChanged || centerChanged || dragging) {
            ...
            if (this.previousExtent != null) {
              var scale = this.previousExtent.getWidth() / bounds.getWidth();
              var fromCenter = this.getPixelFromLonLat(this.previousExtent.getCenterLonLat());
              var toCenter = this.getPixelFromLonLat(bounds.getCenterLonLat());
              var dx = fromCenter.x - toCenter.x;
              var dy = fromCenter.y - toCenter.y;
              if (Math.abs(dx) > 1 || Math.abs(dy) > 1 || Math.abs(scale - 1) > 0.00001) {
                var size = this.getSize();
                var x0 = (1 - scale) * size.w;
                var y0 = (1 - scale) * size.h;
                imageTransform = {x: dx, y: dy, scale: scale, x0: x0, y0: y0};
                this.events.triggerEvent("viewChanged");
              }
            };
            ...
            for (var i=this.layers.length-1; i>=0; --i) {
                ...
                if (layer !== this.baseLayer && !layer.isBaseLayer) {
                    ...
                    if (inRange && layer.visibility) {
                        layer.moveTo(bounds, zoomChanged, options.dragging, imageTransform);
        ...
        this.previousExtent = this.getExtent();
    },
